...
public class ML0_9XQueryBuilder extends AbstractXQueryBuilder implements XQueryBuilder {
	
	// default visibility (package-private), intended that only the factory instantiates this class
	ML0_9XQueryBuilder() {
		super(new StringBuilder("xquery version \"0.9-ml\"\n"));
	}
    ...
    public XQueryBuilder defineExternalVariable(String name, XdmValue value) {
		sb.append("define variable $").append(name);
    ...
    public XdmVariable defineExternalStringVariable(String name, String value) {
        sb.append("define variable $").append(name).append(" as ");
    ...
    public XdmVariable defineExternalNodeVariable(String name, String value) {
        ...
        sb.append("define variable $").append(externalName).append(" as xs:string external\n");
        sb.append("define variable $").append(name).append(" as ");
        sb.append("node() ");
        sb.append("{ ").append("xdmp:unquote($").append(externalName);
        sb.append(")/node()[1] }\n");
