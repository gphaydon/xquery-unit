...
declare function local:test-get-query() {
    let $query as cts:query? := search:get-query("foobar")
    let $expected := cts:word-query("foobar")
    return xq:assert-equal("test-get-query", document{$query}, document{$expected})
};
declare function local:test-get-query-empty() {
    let $query as cts:query? := search:get-query(())
